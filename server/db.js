...
async function getUserByUsername(username) {
  ...
  const users = await supabaseRequest(`users?username_key=eq.${key}&select=*&limit=1`, {
    prefer: "",
  });
  return users?.[0] || null;
}
...
async function getUserById(id) {
  const users = await supabaseRequest(`users?id=eq.${encodeURIComponent(id)}&select=*&limit=1`, {
    prefer: "",
  });
  return users?.[0] || null;
}
...
async function requireUser(req, res) {
  ...
  if (!user) {
    ...
    sendJson(res, 401, { message: "계정을 찾을 수 없습니다." });
    return null;
  }
